...
def import_font(font):
    """ imports one of the three fonts given below

    :param font: {var} one of three list of lists Fonts # Font1, Font2 or Font3
    :returns: {np.ndarray} numof characters x 35 numpy matrix, one line per character
    """
    dataset = np.array([])
    # iterate over all data samples (here 32 characters)
    for i in range(len(font)):
        temp = np.array([])
        # iterate over all hex numbers in a data sample(here 7 hex numbers oftype 0x04 representing 0000 0100 in binary)
        for j in range(len(font[i])):
            # discard first two characters of bin number string representation '0b' [:2] and do zero padding to get len5
            bits = bin(font[i][j])[2:].zfill(5)
            # concatenate all (here 7) len5 binary numbers with data type list[int]
            for charIndex in range(len(bits)):
                temp = np.append(temp, int(bits[charIndex]))
        # append len35 int list representation of data sample to dataset
